...
export async function executeCode(code: string, { actionState, input, sources, ...rest } = {} as ExecuteCodeOptions) {
    ...
    async function waitForInput() {
       return CodeFunctions.waitForInput();
    }
    ...
    function replaceContentStream(token, replaceText) {
        CodeFunctions.replaceContentStream(token, replaceText);
    }
    ...
    async function displayCursorContent(content, encoded) {
       CodeFunctions.displayCursorContent(content, encoded);
    }
...
export function onTaskRecordChange(isRecording: boolean) {
    rec(isRecording);
}
let rec: any;
export function addonTaskRecordChangeListener(func: any) {
    rec = func;
}
